#!/usr/bin/env python3
"""
MindCore AI -- Female Cinematic Pipeline v2.0
=============================================
CHANGES (v2.0):
  ...
  - Female colour grade: warm, soft, intimate -- amber-rose tones.
  - Schedule: Mon-Fri 13:00 UTC (5 videos/week, 4h before male pipeline).
...
# Female cinematic voice (Fish Audio)
...
# Female colour grade: warm, soft, intimate -- amber/rose tones
# eq: gentle contrast, barely brighter, lightly desaturated (keeps natural warmth)
# colorbalance: push red/amber into shadows, cool down blues
COLOR_GRADE_FILTER = (
...
AD_TOPICS = [
    {"pain_point":"the 3am overthinking spiral that just won't stop",
     ...
     "feature":"MindCore AI gives you a private, calm space to untangle what's going through your head."},
    ...
    {"pain_point":"anxiety that gets dismissed as being too emotional",
     "insight":"Women's anxiety often gets minimised. But it's real, and it deserves real support.",
    ...
    {"pain_point":"the guilt that comes with finally putting yourself first",
     ...
     "feature":"MindCore AI is a judgment-free space where putting yourself first isn't selfish."},
...
def load_niche_keywords():
    if not KEYWORDS_PATH.exists():
        return {"seed_queries":["women mental health tips"],"visual_styles":[]}
    with open(KEYWORDS_PATH) as f: return json.load(f)

def pick_visual_style(keywords):
    styles = keywords.get("visual_styles", [])
    if not styles:
        return {"name":"quiet_reflection",
                "query_templates":["woman alone window morning","woman journaling soft light",
                                   "woman sitting quietly nature","soft light woman peaceful"]}
...
def rank_and_select_keyword_claude(candidates, client, topic_history, visual_style):
    ...
    sd = visual_style.get("description","soft introspective")
    st = visual_style.get("query_templates",["woman alone window","woman journaling","soft light woman","woman sitting quietly"])
...
{history_note}
FAVOUR: questions women type privately at 11pm. Emotional phrases. Unsaturated niches.
VISUAL STYLE: {sn} ({sd})
...
def fetch_trending_topic_claude_fallback(seeds, topic_history, visual_style, client):
    ...
    sn   = visual_style.get("name","quiet_reflection")
    st   = visual_style.get("query_templates",["woman alone window","woman journaling morning","woman walking forest","soft light woman thinking"])
...
Write a cinematic voiceover script for MindCore AI targeting women.
...
def ken_burns_vf(clip_duration: float, direction: str) -> str:
    """
    Returns a scale+crop FFmpeg filter string for a slow pan or zoom.
    Overscan by KB_SCALE (10%) to allow movement without black borders.
    """
    d  = max(clip_duration, 0.5)
    sw = int(1080 * KB_SCALE)   # 1188
    sh = int(1920 * KB_SCALE)   # 2112
    ew = sw - 1080               # 108 px extra width
    eh = sh - 1920               # 192 px extra height
...
def main():
    ...
    cfg    = {}
    cfg_path = PIPELINE_DIR / "heygen_config_female.json"
    if cfg_path.exists():
    ...
    print(f"  Colour grade: FEMALE (warm, soft, intimate)")
    ...
    pexels_queries = script.get("pexels_queries") or [
        "woman alone window morning light","woman journaling soft light",
        "woman sitting quietly nature","soft light hands tea cup","woman peaceful moment"
    ]
