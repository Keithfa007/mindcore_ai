#!/usr/bin/env python3
"""
MindCore AI Video Pipeline -- FEMALE v1.0
==========================================

Mirrors heygen_pipeline.py (male v5.14) but targets women's mental health.

Key differences:
  - Female avatar looks (13 in shuffled deck)
  - Female Fish Audio voice: 072bc615ad46485889e4eecc823985c5
  - Female-specific SERP seed queries
  - Female-specific content angles and visual styles
  - Script persona: credible woman in her late 30s, podcast interview format
  - Script audience: Women 25-45, overwhelmed, overthinking, people pleasing
  - Female-specific ad pain points
  - Separate config, look queue, topic history files
  - Same: Whisper captions, cropdetect limit=200, Upload-Post 4 platforms
...
# Female-specific Fish Audio voice
...
LOOK_QUEUE_PATH       = PIPELINE_DIR / "look_queue_female.json"
CONFIG_PATH           = PIPELINE_DIR / "heygen_config_female.json"
...
AD_TOPICS = [
    ...
    },
    {
        ...
        "insight":    "Women's anxiety often gets minimised -- by others and by themselves. 'I'm just overthinking.' 'I'm being too sensitive.' But it's real, and it deserves real support.",
    ...
    },
    {
        ...
        "feature":    "MindCore AI is there for the moments when you need to be heard and there's nobody to call. Private, available 24/7, built for this exact moment.",
    },
    {
    ...
    },
    {
        ...
        "insight":    "Most women are experts at looking okay when they're not. The performance becomes exhausting -- and isolating, because nobody knows to check in.",
    ...
    },
    {
        ...
        "feature":    "MindCore AI helps you understand the patterns underneath and find a way back to yourself. Available any time, no waiting list, on Google Play.",
    },
]
...
def load_look_queue(all_looks: list) -> list:
    ...
    deck = all_looks[:]
    random.shuffle(deck)
    print(f"  Look queue: new shuffled deck of {len(deck)} female looks")
    return deck
...
def pick_avatar_look(cfg: dict) -> str:
    ...
    if not all_looks:
        raise RuntimeError("No avatar_look_ids found in heygen_config_female.json")
...
def load_niche_keywords() -> dict:
    if not KEYWORDS_PATH.exists():
        return {"seed_queries": ["women mental health tips"], "content_angles": ["real talk"], "visual_styles": []}
...
def pick_visual_style(keywords: dict) -> dict:
    ...
    if not styles:
        return {"name": "quiet_reflection", "query_templates": ["woman alone window", "woman journaling", "woman sitting alone"]}
...
def research_keyword_candidates_from_serp(seeds: list) -> list:
    ...
    for seed in regular_seeds:
        try:
            ...
            total_results = int(str(data.get("search_information", {}).get("total_results", "0")).replace(",", "").replace(".", "") or "0")
            paa_count = 0
            for q in data.get("related_questions", []):
                text = q.get("question", "").strip()
                if text and text.lower() not in seen:
                    seen.add(text.lower())
                    candidates.append({"text": text, "source": "people_also_ask", "tail_type": _keyword_type(text), "word_count": _word_count(text), "seed": seed, "total_results": total_results})
                    paa_count += 1
            rs_count = 0
            for r in data.get("related_searches", []):
                ...
                if text and text.lower() not in seen:
                    seen.add(text.lower())
                    candidates.append({"text": text, "source": "related_search", "tail_type": _keyword_type(text), "word_count": _word_count(text), "seed": seed, "total_results": 0})
                    rs_count += 1
            for org in data.get("organic_results", [])[:3]:
                title = org.get("title", "").strip()
                if title and title.lower() not in seen and len(title) < 120:
                    seen.add(title.lower())
                    candidates.append({"text": title, "source": "organic_title", "tail_type": _keyword_type(title), "word_count": _word_count(title), "seed": seed, "total_results": total_results})
    ...
    for ac_seed in ac_seeds:
        ...
        for text in suggestions:
            if text and text.lower() not in seen and _word_count(text) <= 6:
                seen.add(text.lower())
                candidates.append({"text": text, "source": "autocomplete", "tail_type": _keyword_type(text), "word_count": _word_count(text), "seed": ac_seed, "total_results": 0})
                ac_count += 1
        if ac_count:
...
def rank_and_select_keyword_claude(candidates: list, client: anthropic.Anthropic, topic_history: list, visual_style: dict) -> dict:
    if not candidates:
    ...
    style_templates = visual_style.get("query_templates", ["woman alone window", "woman journaling", "woman sitting alone"])
    style_name      = visual_style.get("name", "quiet_reflection")
    style_desc      = visual_style.get("description", "soft introspective")

    prompt = f"""Expert in SEO for women's mental health, anxiety, burnout, relationships on TikTok/Reels/YouTube Shorts.

Below are REAL Google search queries. Choose the SINGLE BEST keyword for a short video today.
{history_note}
FAVOUR: questions women actually ask out loud or search privately.
Short emotional phrases (anxiety women, people pleasing, burnout, overthinking) perform best.
Big brands ignore raw emotional short phrases -- individual creators own this space.

SCORING:
1. Would a woman type this at 11pm when she can't sleep?
2. Emotional resonance for women 25-45 struggling silently
3. Low competition: under big-brand radar?
4. Niche fit: women's mental health, anxiety, burnout, self-worth, relationships

FORMAT DECISION:
- "avatar": questions, direct advice, personal testimony, how-to -- PREFERRED
- "cinematic": abstract emotional states, reflective, atmospheric
...
{{
  ...
  "question": "the exact question a woman would ask -- rephrase as a question if needed",
...
def fetch_trending_topic_claude_fallback(seeds: list, topic_history: list, visual_style: dict, client: anthropic.Anthropic) -> dict:
    seed       = random.choice(seeds)
    hist_note  = f"AVOID: {', '.join(topic_history)}. " if topic_history else ""
    style_name = visual_style.get("name", "quiet_reflection")
    prompt = f"""SEO expert for women's mental health, anxiety, burnout, relationships.
Generate ONE question/topic for a short interview-style answer video. Related to: "{seed}"
{hist_note}Return ONLY valid JSON:
{{
  "topic": "the question or keyword",
  "question": "the exact question a woman would ask -- must be a question",
  "keyword": "primary 1-5 word SEO keyword",
  "tail_type": "short_tail|mid_tail|long_tail",
  "competition_signal": "low|medium|high",
...
# ---------------------------------------------------------------------------
# Script Generation -- Female persona, interview response format
# ---------------------------------------------------------------------------

def generate_content_script(topic: dict, client: anthropic.Anthropic) -> dict:
    print(f"  Generating FEMALE INTERVIEW RESPONSE script for: {topic['topic']}")
    ...
    cinematic_note = "\nNOTE: This is a voiceover for cinematic B-roll. Write for the ear only." if fmt == "cinematic" else ""
    ...
    prompt = f"""You are a warm, credible woman in her late 30s being interviewed on a podcast about women's mental health and emotional wellness.
...
3. story: The truth most women relate to but nobody says.
4. solution_cta: Genuine takeaway. What a woman can actually do or understand.

AUDIENCE: Women 25-45, exhausted, overwhelmed, often putting everyone else first.
Typically juggling work, relationships, family -- while quietly struggling inside.
SEO KEYWORD: {keyword} (weave in naturally)

TONE: Warm, direct, zero judgment. Like a trusted older sister who gets it.
- Short sentences when they land harder.
- Natural pauses: "And that's the thing." / "You know?"
...
def generate_ad_script(app_facts: dict, client: anthropic.Anthropic) -> dict:
    ...
    hook_style = random.choice(INTERVIEW_HOOKS)
    print(f"  Generating FEMALE AD script...")
...
Feel like content for the first two scenes. Audience: women 25-45.

PAIN POINT: {ad_topic['pain_point']}
INSIGHT: {ad_topic['insight']}
FEATURE: {ad_topic['feature']} (also: private, 24/7, for women, Google Play)

SCENES:
- hook: Stops scroll. No MindCore AI. Pure emotional truth.
- problem: Expands insight. No MindCore AI.
- story: Introduce MindCore AI naturally. "There's something called MindCore AI..."
...
def submit_heygen_video(script_text: str, avatar_id: str, voice_id: str) -> str:
    ...
    MOTION_PROMPT = (
        "Gesturing naturally with hands while speaking. "
        "Warm, direct eye contact. Nodding gently on emotional points. "
        "Open, welcoming gestures. Natural, conversational energy throughout."
    )
    payload = {
        "type":                "avatar",
        "avatar_id":           avatar_id,
...
def generate_fish_audio_tts(script_text: str, output_path: str) -> str:
    ...
    print(f"  Fish Audio TTS (female voice): {FISH_AUDIO_VOICE_ID[:8]}... | {len(script_text)} chars")
        ...
        for orientation in ("portrait", None):
            ...
            try:
                ...
                for video in resp.json().get("videos", []):
                    ...
                    if chosen_files:
                        clips.append({"url": chosen_files[0]["link"], "query": query, "id": vid_id, "duration": video.get("duration", 10)})
                        if len(clips) >= num_clips:
                            break
                time.sleep(0.3)
            except Exception as e:
...
Hashtags should be women's mental health focused: #womensmentalhealth #anxietywomen #burnout etc."""
    for attempt in range(1, CLAUDE_MAX_RETRIES + 1):
        try:
...
def generate_upload_metadata(script: dict, mode: str, client: anthropic.Anthropic) -> dict:
    ...
    prompt = f"""Social media expert for women's mental health on TikTok, Instagram, Facebook and YouTube Shorts.

VIDEO TYPE: {video_type}
QUESTION THIS VIDEO ANSWERS: {question}
SEO KEYWORD: {seo_kw}
OPENING LINE: {hook_vo}

Write platform metadata. CRITICAL RULES:
- ALL descriptions must be original sentences written from scratch.
- Do NOT copy, quote, or paraphrase the video script.
- tiktok_caption: 1-2 punchy sentences + 8-10 hashtags.
  Max 2200 chars. MUST include: {REQUIRED_BRAND_HASHTAG} #womensmentalhealth
- facebook_title: max 255 chars, phrase as the question
- facebook_description: 2 original sentences + question + 4-5 hashtags. MUST include {REQUIRED_BRAND_HASHTAG}
- youtube_title: max 100 chars -- phrase as the question women search for
- youtube_description: 2 short sentences. Then blank line. Then: "Try MindCore AI: https://mindcoreai.eu"
  Then blank line. Then 6-8 hashtags ending with #Shorts. MUST include {REQUIRED_BRAND_HASHTAG}
- youtube_tags: comma-separated 8-12 keywords (no # symbols, women's mental health focused)

Return ONLY valid JSON:
{{
  "tiktok_caption": "hook or question... {REQUIRED_BRAND_HASHTAG} #womensmentalhealth #hashtag",
  "facebook_title": "...",
  "facebook_description": "2 sentences. {REQUIRED_BRAND_HASHTAG} #hashtag",
  "youtube_title": "...",
...
def save_upload_guide(guide_text: str, script: dict, mode: str, run_number: int, render_fmt: str):
    ...
    header = f"""================================================================================
  MINDCORE AI -- FEMALE VIDEO UPLOAD GUIDE
  Run #{run_number} | {generated_at}
================================================================================
  Video type      : {video_type}
  Format          : {render_fmt.upper()}
...
"""
    ...
    (OUTPUT_DIR / "upload_guide_female.txt").write_text(full, encoding="utf-8")
...
def main():
    ...
    print(f"\n  MindCore AI -- FEMALE Video Pipeline v1.0")
    ...
    print(f"  Music library: {len(music_tracks)} track(s) @ {int(MUSIC_VOLUME * 100)}%")
    ...
    if mode == "ad":
        ...
        pexels_queries = topic.get("pexels_queries", ["woman alone window", "woman journaling", "soft light woman thinking"])
    ...
    if est_duration > 60:
        print(f"  NOTE: >{est_duration}s -- YouTube posts as regular video, not Short.")
    print()
    for scene in SCENE_ORDER:
        wc = len(script[scene]["voiceover"].split())
    ...
    if render_fmt == "cinematic":
        print(f"\n  Rendering CINEMATIC video (female voice)...")
        try:
