...
# ---------------------------------------------------------------------------
# Subtitle generation -- Whisper + ASS (AVATAR + CINEMATIC)
# ---------------------------------------------------------------------------
...
def load_niche_keywords() -> dict:
    if not KEYWORDS_PATH.exists(): return {"seed_queries": ["women mental health tips"], "content_angles": ["real talk"], "visual_styles": []}
    with open(KEYWORDS_PATH) as f: return json.load(f)

def pick_visual_style(keywords: dict) -> dict:
    styles = keywords.get("visual_styles", [])
    if not styles: return {"name": "quiet_reflection", "query_templates": ["woman alone window", "woman journaling", "woman sitting alone"]}
...
# ---------------------------------------------------------------------------
# Script Generation -- Female persona
# ---------------------------------------------------------------------------
...
def generate_ad_script(app_facts: dict, client: anthropic.Anthropic) -> dict:
    ...
    print(f"  Generating FEMALE AD script... pain point: {ad_topic['pain_point'][:65]}...")
...
def render_avatar_video(script_text: str, cfg: dict) -> str:
    ...
    ass_path = str(OUTPUT_DIR / "subtitles_female_avatar.ass")
