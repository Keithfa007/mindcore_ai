...
FACEBOOK_TITLE = "Why I Built MindCore AI — A Private AI Companion for Anyone Who Carries It Alone"
...
def download_video(url: str, path: str):
    ...
    print(f"  Downloaded: {path} ({Path(path).stat().st_size/(1024*1024):.1f} MB)")
...
def detect_crop(path: str) -> tuple:
    cmd=["ffmpeg","-i",path,"-vf","cropdetect=limit=200:round=2:reset=0","-frames:v","90","-f","null","-"]
...
def to_portrait(raw: str, final: str, ass_path: str = None):
    w,h=get_dimensions(raw);print(f"  Raw dimensions: {w}x{h}")
    ...
    cmd=["ffmpeg","-i",raw,"-vf",filter_str,"-c:v","libx264","-crf","16","-preset","slow","-b:v","4M","-maxrate","6M","-bufsize","8M","-c:a","copy","-y",final]
    result=subprocess.run(cmd,capture_output=True,text=True)
    if result.returncode!=0:
        if ass_path and Path(ass_path).exists():
            print("  WARNING: subtitle burn failed -- retrying without captions")
            to_portrait(raw,final,ass_path=None);return
        raise RuntimeError(f"ffmpeg failed:\n{result.stderr[-500:]}")
    w2,h2=get_dimensions(final);size=Path(final).stat().st_size/(1024*1024)
...
def upload_to_platforms(video_path: str) -> dict:
    ...
    data=[
        ...
        ("title",TIKTOK_CAPTION[:2200]),("facebook_title",FACEBOOK_TITLE[:255]),
        ("facebook_description",FACEBOOK_DESCRIPTION[:2000]),
        ("youtube_title",YOUTUBE_TITLE[:100]),("youtube_description",YOUTUBE_DESCRIPTION[:5000]),
        ("youtube_tags",YOUTUBE_TAGS),
    ]
    try:
        with open(video_path,"rb") as f:
            files=[("video",("mindcore_intro.mp4",f,"video/mp4"))]
            resp=requests.post(UPLOAD_POST_API_URL,headers=headers,files=files,data=data,timeout=300)
        result=resp.json() if resp.headers.get("content-type","").startswith("application/json") else {"raw":resp.text}
        result["status_code"]=resp.status_code
        if resp.ok: print(f"  Upload successful: {resp.status_code}")
        else: print(f"  Upload WARNING: {resp.status_code} -- {resp.text[:300]}")
        return result
    except Exception as e:
