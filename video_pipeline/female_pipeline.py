...
=============================================
v5.0: Pexels B-roll restored, fal.ai removed, no background music.
      Voice only -- TikTok adds its own audio, FB/YouTube get raw voice.
...
WORD_FLASH_STOPWORDS = {'the','and','for','are','but','not','you','all','can','had','her','was','one','our','out','get','has','him','his','how','its','may','new','now','old','see','two','way','who','did','let','put','say','she','too','use','a','i','in','is','it','if','do','an','so','at','be','by','no','or','on','up','as','of','to','from','that','this','with','have','they','what','when','your','will','been','were','just','than','then','them','these','into','some','there','about','which','their','after','every','where','would','could','should','still','going','really','never','always','here','more','very','even','also','back','down','only','look','come','want','give','most','know','take','think','good','like','time','feel','make','right','both','each','much','well','dont','doesnt','didnt','wont','cant','its','youre','theyre','were','im','thats','theres','heres','ive','youve','weve'}
POWER_WORDS = {'broken','tired','alone','lost','numb','empty','heavy','dark','scared','hurt','pain','shame','guilt','anger','grief','fear','doubt','silence','hollow','drained','invisible','worthless','hopeless','desperate','trapped','healing','depression','anxiety','trauma','burnout','exhausted','overwhelmed','suffocating','withdrawn','sensitive','anxious','fight','carry','hold','fall','rise','change','truth','real','survive','breathe','pretend','mask','hide','quit','disappear','enough','worthy','seen','heard','free','strong','silent','soul','heart','weight','burden','purpose','missing','love','pretending','performing','disappearing','shrinking','drowning','fading','pleasing','hiding','carrying','protecting','managing','fixing','wound','depleted','difference','flaw','earned','rooms','wrong','apologize','perception','rehearse','glass'}

HOOK_FORMULAS = [
    {"name":"confession_ladder","instruction":"Open at the emotional floor with a 3-4 beat escalating confession. Each beat names one dimension of exhaustion (emotional, mental, physical, spiritual). Drop the viewer mid-confession -- zero setup, zero preamble.","example":"Emotionally, I'm completely empty. Mentally, I can't hold one more thought. And physically? I'm so tired I can't even cry anymore.","rule":"Each beat must name a different dimension. The ladder MUST escalate in emotional weight. Open as if mid-confession -- no 'Have you ever felt' or any setup. The viewer hears herself in the first word."},
    ...
    {"name":"specific_number","instruction":"Open with a specific, unexpected statistic or number that reframes how common the experience is. The number IS the hook.","example":"7 in 10 women feel like they're performing a version of themselves every single day.","rule":"Use odd, specific numbers -- never round numbers like 10 or 20. The number must make the viewer think: I didn't know it was that many."},
]
...
def get_scheduled_post_time():
    ...
    scheduled=target.strftime("%Y-%m-%dT%H:%M:%SZ"); print(f"  Scheduled: {scheduled} ({POST_HOUR_UTC:02d}:00 UTC = {POST_HOUR_UTC+2:02d}:00 Malta)"); return scheduled
...
def sanitize_script(script):
    for scene in SCENE_ORDER:
        ...
        for p,r in BANNED_PHRASE_REPLACEMENTS: cleaned=re.sub(p,r,cleaned,flags=re.IGNORECASE)
...
def load_keywords_data():
    if not KEYWORDS_PATH.exists(): return {"schedule":{},"niches":{"default":{"name":"Women's Mental Health","viewer_persona":"A woman in her 30s carrying everything for everyone.","seed_queries":["women mental health tips"],"hashtags":[],"visual_moods":[{"name":"default","description":"soft warm","pexels_queries":[]}]}}}
...
def _serp_google_query(seed):
    r=requests.get(SERP_API_URL,params={"engine":"google","q":seed,"api_key":SERP_API_KEY,"num":10,"hl":"en","gl":"us"},timeout=30); r.raise_for_status(); return r.json()
def _serp_autocomplete_query(seed):
    try: r=requests.get(SERP_API_URL,params={"engine":"google_autocomplete","q":seed,"api_key":SERP_API_KEY,"hl":"en","gl":"us"},timeout=30); r.raise_for_status(); return [s.get("value","").strip() for s in r.json().get("suggestions",[]) if s.get("value")]
...
def research_keyword_candidates_from_serp(seeds):
    ...
    for seed in random.sample(seeds,min(SERP_SEEDS_PER_RUN,len(seeds))):
        try:
            ...
            for q in data.get("related_questions",[]): t=q.get("question","").strip();
            ...
            for r in data.get("related_searches",[]): t=r.get("query","").strip();
            ...
            for org in data.get("organic_results",[])[:3]: t=org.get("title","").strip();
    ...
    for ac in random.sample(list(set(bases)),min(AUTOCOMPLETE_SEEDS_PER_RUN,len(set(bases)))):
        ...
        if ac_count: print(f"  [AUTOCOMPLETE] '{ac}': {ac_count} suggestions"); time.sleep(0.5)
...
def fetch_trending_topic(client,niche):
    ...
    if SERP_API_KEY:
        try:
            ...
            if cands:
                topic=rank_and_select_keyword_claude(cands,client,th,niche); topic["source"]=f"serp_{topic.get('source','research')}"
                (OUTPUT_DIR/"keyword_research.json").write_text(json.dumps({"run":GITHUB_RUN_NUMBER,"niche":niche["name"],"candidates":cands,"winner":topic},indent=2)); return topic
...
def generate_ad_script(app_facts,niche,client):
    ...
    prompt=f"""You are writing a punchy cinematic voiceover ad script for MindCore AI targeting women. Target: 25-35 seconds.\n\nVIEWER: {niche['viewer_persona']}\nPAIN POINT: {at['pain_point']}\nINSIGHT: {at['insight']}\nFEATURE: {at['feature']} (private, 24/7, Google Play)\n\n{hb}\n\nSCENES (KEEP TIGHT):\nhook -> problem ({lp}-{hp} words) -> story ({ls}-{hs} words, introduce MindCore AI naturally) -> solution_cta ({lc}-{hc} words -- mention Google Play briefly, then end with emotional resolution or engagement trigger. Match tone.)\nBANNED: "free trial", "first week free", "download now"\n\nReturn ONLY valid JSON:\n{{"video_type":"ad","topic":"{at['pain_point'][:55]}","seo_keyword":"AI mental health companion for women","render_format":"cinematic","hook_formula":"{f['name']}","hook":{{"voiceover":"..."}},"problem":{{"voiceover":"..."}},"story":{{"voiceover":"..."}},"solution_cta":{{"voiceover":"..."}}}}"""
    return _call_claude_raw(prompt,client,max_tokens=800)

def build_full_script(script):
    parts=[]
    for s in SCENE_ORDER: vo=script[s]["voiceover"].strip(); vo+="." if vo and vo[-1] not in ".!?" else ""; parts.append(vo)
    return "  ".join(parts)

def pick_power_word(wic):
    cands=[w for w in wic if len(w["word"].strip())>2 and w["word"].strip().lower().rstrip(".,!?'\"") not in WORD_FLASH_STOPWORDS and w["word"].strip().replace("'","").replace("-","").isalpha()]
    if not cands: return None
    pw=[w for w in cands if w["word"].strip().lower().rstrip(".,!?'\"") in POWER_WORDS]
    if pw: return pw[0]["word"].strip().upper()
    return max(cands,key=lambda w:len(w["word"].strip()))["word"].strip().upper()
...
def transcribe_audio_whisper(mp):
    try:
        ...
        for seg in result.get("segments",[]):
            for w in seg.get("words",[]): word=w.get("word","").strip();
...
def generate_ass_subtitles(words,output_path):
    ...
    for idx,chunk in enumerate(chunks):
        ...
        if word: flash_events+=f"Dialogue: 0,{ts(chunk['start'])},{ts(chunk['end'])},Flash,,0,0,0,,{word}\n"; fc+=1
    Path(output_path).write_text((header+events+flash_events).encode("utf-8",errors="ignore").decode("utf-8"),encoding="utf-8")
...
def burn_subtitles_into_video(vp,ap):
    ...
    print("  WARNING: subtitle burn failed"); Path(bt).exists() and Path(bt).unlink(); return False
...
def generate_fish_audio_tts(st,op):
    ...
    h={"Authorization":f"Bearer {FISH_AUDIO_API_KEY}","Content-Type":"application/json"}
    p={"text":st,"reference_id":FISH_AUDIO_VOICE_ID,"format":"mp3","mp3_bitrate":192,"latency":"normal"}
    print(f"  Fish Audio TTS: {FISH_AUDIO_VOICE_ID[:8]}... | {len(st)} chars")
    r=requests.post(FISH_AUDIO_TTS_URL,headers=h,json=p,stream=True,timeout=120)
    if not r.ok: raise RuntimeError(f"Fish Audio TTS failed {r.status_code}: {r.text[:300]}")
...
def process_clip_to_portrait(cp,op,dur,direction="pan_right",cg=None):
    ...
    vf=f"{ken_burns_vf(dur,direction)},{cg},fps=30"
    r=subprocess.run(["ffmpeg","-stream_loop","-1","-i",cp,"-vf",vf,"-t",str(dur),"-an","-c:v","libx264","-crf","20","-preset","fast","-y",op],capture_output=True,text=True)
...
def assemble_cinematic_video(clip_paths,audio_path,output_path,music_path=None,ass_path=None,scene_types=None):
    ...
    if music_path:
        cmd=["ffmpeg","-i",cv,"-i",audio_path,"-stream_loop","-1","-i",music_path,"-filter_complex",f"[2:a]volume={MUSIC_VOLUME}[music];[1:a][music]amix=inputs=2:duration=first:normalize=0[aout]","-map","0:v","-map","[aout]","-c:v","copy","-c:a","aac","-b:a","192k","-t",str(ad),"-y",output_path]
    else:
        cmd=["ffmpeg","-i",cv,"-i",audio_path,"-map","0:v:0","-map","1:a:0","-c:v","copy","-c:a","aac","-b:a","192k","-t",str(ad),"-y",output_path]
    r=subprocess.run(cmd,capture_output=True,text=True)
    if r.returncode!=0:
        if music_path: print("  Music mix failed -- retrying without"); assemble_cinematic_video(clip_paths,audio_path,output_path,music_path=None,ass_path=ass_path,scene_types=scene_types); return
        raise RuntimeError(f"Audio mix failed: {r.stderr[-500:]}")
    print(f"  Assembled: {get_video_dimensions(output_path)[0]}x{get_video_dimensions(output_path)[1]} | {Path(output_path).stat().st_size/(1024*1024):.1f} MB")
...
def render_cinematic_video(script_text,mood,niche,script=None):
    ...
    for scene_name,count in [("hook",1),("problem",2),("story",1),("solution_cta",1)]:
        for j in range(count):
            ...
            pexels_path=fetch_pexels_clip_for_scene(scene_name,len(raw_clip_paths),clip_path,GITHUB_RUN_NUMBER,gender="woman")
    ...
    music_path = None  # Voice only -- no background music. TikTok adds its own audio.
...
def get_video_dimensions(p):
    parts=subprocess.run(["ffprobe","-v","error","-select_streams","v:0","-show_entries","stream=width,height","-of","csv=p=0",p],capture_output=True,text=True,check=True).stdout.strip().split(",")
...
def generate_upload_metadata(script,mode,niche,client):
    ...
    for a in range(1,CLAUDE_MAX_RETRIES+1):
        try:
            ...
            print(f"  TikTok:  {md.get('tiktok_caption','')[:80]}..."); print(f"  YouTube: {md.get('youtube_title','')[:60]}..."); print(f"  Comment: {md.get('first_comment','')}"); return md
...
def upload_to_platforms(video_path,metadata,cfg,scheduled_date=None):
    ...
    try:
        with open(video_path,"rb") as f:
            resp=requests.post(UPLOAD_POST_API_URL,headers=headers,files=[("video",("mindcore_female_video.mp4",f,"video/mp4"))],data=data,timeout=180)
...
def save_upload_guide(gt,script,mode,rn,niche):
    ...
    (OUTPUT_DIR/"upload_guide_female.txt").write_text(header+gt,encoding="utf-8"); print("  Upload guide saved")
...
def main():
    ...
    print(f"\n  MindCore AI -- Female Cinematic Pipeline v5.0")
    print(f"  Run #{GITHUB_RUN_NUMBER} | Mode: {mode.upper()} | Pexels B-roll | Voice only (no music)")
    print(f"  Niche: {niche['name']} | Voice: {FISH_AUDIO_VOICE_ID[:8]}...")
    print(f"  Post: {POST_HOUR_UTC:02d}:00 UTC = {POST_HOUR_UTC+2:02d}:00 Malta | Upload: {'ENABLED' if upload_enabled else 'DISABLED'}")
    print("="*60)
    print("\n  Generating script...")
