...
class _MoodHistoryScreenState extends State<MoodHistoryScreen> {
  ...
  double _avgMoodOf(List<MoodEntry> list) {
    ...
    return list.fold<int>(0, (a, e) => a + e.mood) / list.length;
  }
  ...
  int _bestStreakOf(List<MoodEntry> list) {
    ...
    for (final e in list) {
      ...
      byDay[key] =
          (byDay[key] ?? 0) < e.mood ? e.mood : (byDay[key] ?? e.mood);
    }
  ...
  List<Widget> _buildMoodGroupWidgets(List<MoodEntry> entries) {
    ...
    for (final e in entries) {
      (byDay[DateFormat('yyyy-MM-dd').format(e.timestamp)] ??= []).add(e);
    }
            ...
            Wrap(
              ...
              children: [
                _TopTabChip(
                  ...
                  onTap: () =>
                      setState(() => _tab = HistoryTab.progress),
                ),
                _TopTabChip(
                  label: 'Mood Logs',
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: [
                        ...
                        const SizedBox(height: 6),
                        Text(
                          'Tip: Multiple logs per day are fine — bar height is the daily average.',
    ...
    return AnimatedBackdrop(
      child: GestureDetector(
        ...
        onHorizontalDragEnd: (d) {
          ...
          if (vx < -100) {
            setState(() => _monthCursor = DateTime(
            ...
            GlassCard(
              child: Column(
                ...
                children: [
                  ...
                  _MonthHeader(
                    month: _monthCursor,
                    onPrev: () => setState(() => _monthCursor = DateTime(
                        _monthCursor.year, _monthCursor.month - 1, 1)),
                    onNext: () => setState(() => _monthCursor = DateTime(
                        _monthCursor.year, _monthCursor.month + 1, 1)),
                  ),
                  const SizedBox(height: 8),
                  _SummaryRow(
...
class _MoodCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final day =
        DateFormat('EEE, MMM d • HH:mm').format(entry.timestamp);
    return Container(
...
class _DailyMoodBars extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final itemWidth =
        (labelMode == _BarLabelMode.month) ? 56.0 : 64.0;
      ...
      child: _scrollable
          ? SingleChildScrollView(
              ...
              child: Row(
                ...
                children: points
                    .map((p) =>
                        _barItem(context, p, barAreaH, width: itemWidth))
                    .toList(),
              ),
            )
          : Row(
              ...
              children: points
                  .map((p) => Expanded(
                      child: _barItem(context, p, barAreaH)))
                  .toList(),
            ),
    );
  ...
  Widget _labelsRow(BuildContext context, double itemWidth) {
    final theme = Theme.of(context);
    Widget labelFor(DateTime d) => Text(
          _labelFor(d, labelMode),
          maxLines: 1,
  ...
  Widget _barItem(BuildContext context, _WeeklyPoint p, double maxH,
            ...
            : (p.count <= 1 || p.entries.isEmpty)
                ? Container(
                    color: p.entries.length == 1
                        ? _StackedSlices._segmentColor(
                            p.entries.first.mood)
    ...
    final frame = SizedBox(
      width: 14,
      height: maxH,
      child: Align(alignment: Alignment.bottomCenter, child: barBody),
    );
    ...
    return SizedBox(
      ...
      child: Padding(
        ...
        child:
            Align(alignment: Alignment.bottomCenter, child: tappable),
      ),
    );
  }
    ...
    return Column(
      ...
      children: entries.map((e) {
        ...
        return Expanded(
          flex: flex,
          child: Container(
              width: double.infinity, color: _segmentColor(e.mood)),
        );
      }).toList(),
    );
  }
