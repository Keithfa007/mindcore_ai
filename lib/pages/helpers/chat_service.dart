// lib/pages/helpers/chat_service.dart
import 'dart:async';
import 'dart:convert';
...
import 'chat_persona_prefs.dart';
import 'journal_service.dart';

/// Orchestrated OpenAI chat for MindCore AI.
class ChatService {
  ...
  static Future<OrchestratorReply> generateOrchestratedReply({
    ...
    if (_apiKey.trim().isEmpty) {
      ...
      return const OrchestratorReply(
        reply: text,
        agent: AgentType.companion,
        confidence: 0.35,
        supportModeLabel: 'Calm Companion',
        suggestedActions: [],
        ttsText: text,
      );
    }
    ...
    var system = AgentPrompts.buildSystemPrompt(
      agent: decision.agent,
      context: context,
      personaProfileText: personaProfile.profileText,
    );
    if (systemOverlay.trim().isNotEmpty) {
      system = '$system\n\nAdditional response guidance: ${systemOverlay.trim()}';
    }
    ...
    try {
      ...
      if (res.statusCode != 200) {
        ...
        final text = 'AI request failed (${res.statusCode}). ${err ?? "Please try again."}';
      ...
      final clean = (text == null || text.trim().isEmpty)
  ...
  static Future<String> generateReply({
    ...
    final result = await generateOrchestratedReply(
      history: history,
      moodLabel: moodLabel,
      userInput: userInput,
      screen: screen,
