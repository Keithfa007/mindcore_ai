...
class _ChatScreenState extends State<ChatScreen> with AutoStopTtsRouteAware<ChatScreen> {
  ...
  Future<Map<String, String>?> _showMoodPicker() async {
    ...
    if (res is Map) {
      final emoji = (res['emoji'] ?? '🙂').toString();
      final label = (res['label'] ?? 'Neutral').toString();
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (finalIdx != -1) {
        final existing = _messages[finalIdx];
        _messages[finalIdx] = existing.copyWith(
  ...
  Future<void> _retryAssistantMessage(String messageId) async {
    ...
    int u = idx - 1;
    while (u >= 0 && _messages[u].role != 'user') {
      u--;
    }
    if (u < 0) return;
    ...
    try {
      ...
      final result = await ChatStreamService.streamOrchestratedReply(
        ...
        onDelta: (delta) async {
          if (!mounted || _currentConvId != convIdAtSend) return;
          final currentIdx = _messages.indexWhere((m) => m.id == messageId);
...
extension _TakeLast<E> on List<E> {
  Iterable<E> takeLast(int n) {
    ...
    return getRange(start < 0 ? 0 : start, length);
  }
}
