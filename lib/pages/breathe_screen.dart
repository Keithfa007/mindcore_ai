...
class _BreathePrefs {
  ...
  static const _default = _BreatheSettings(
    ...
    haptics: true, targetCycles: 0, tts: false,
  );
  ...
  static Future<void> save(_BreatheSettings s) async {
    ...
    await p.setBool(_kHapt, s.haptics);  await p.setInt(_kTarget, s.targetCycles);
...
class _BreatheScreenState extends State<BreatheScreen>
  ...
  void _onTick() {
    if (_running) {
      if (_c.value < _lastAnimValue) {
        ...
        if (_settings.targetCycles > 0 && _completedCycles >= _settings.targetCycles) {
          _stopWithTargetReached(); _lastAnimValue = _c.value; return;
        }
      }
    }
  ...
  Future<void> _speakPhase(_Phase p) async {
    if (!_settings.tts || !_running) return;
    ...
    switch (p) {
      ...
      case _Phase.hold1:
      case _Phase.hold2:  text = _aiCoachEnabled ? _coachPlan.hold   : 'Hold';   break;
    }
  ...
  Future<void> _openSettings() async {
    ...
    setState(() {
      ...
      _phaseVN.value = _Phase.inhale; _lastSpokenPhase = null; _lastAnimValue = 0.0;
      if (_running) { _c.repeat(period: _cycle); _speakPhase(_phaseVN.value); }
      else { OpenAiTtsService.instance.stop(); }
    });
  }

  String _phaseLabel(_Phase p) {
    switch (p) { case _Phase.inhale: return 'Inhale'; case _Phase.hold1: case _Phase.hold2: return 'Hold'; case _Phase.exhale: return 'Exhale'; }
  }
  ...
  double _phaseLocalProgress() {
    ...
    final a = _dInhale.inMilliseconds; final b = _dHold1.inMilliseconds;
    final c = _dExhale.inMilliseconds; final d = _dHold2.inMilliseconds;
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(child: Text('Breathe', style: t.titleLarge?.copyWith(fontWeight: FontWeight.w900))),
                ...
                Row(
                  ...
                  children: [
                    Icon(Icons.all_inclusive, size: 18, color: scheme.onSurface.withValues(alpha: 0.6)),
                    const SizedBox(width: 6),
                    ValueListenableBuilder<int>(
                      valueListenable: _cyclesVN,
                      builder: (_, cycles, __) => Text('$cycles cycle${cycles == 1 ? '' : 's'}', style: t.bodySmall?.copyWith(color: scheme.onSurface.withValues(alpha: 0.65), fontWeight: FontWeight.w700)),
                    ),
                    if (_settings.targetCycles > 0) ...[
                      const SizedBox(width: 12),
                      Text('•', style: t.bodySmall?.copyWith(color: scheme.onSurface.withValues(alpha: 0.55))),
                      const SizedBox(width: 12),
...
class _BreatheSettingsSheetState extends State<_BreatheSettingsSheet> {
  ...
  void initState() {
    super.initState();
    _preset = widget.initial.preset; _inh = widget.initial.inhaleS;
    _h1 = widget.initial.hold1S;    _exh = widget.initial.exhaleS;
    _h2 = widget.initial.hold2S;    _haptics = widget.initial.haptics;
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  const SizedBox(width: 12),
                  Expanded(child: GradientButton.primary('Save', onPressed: () {
                    Navigator.pop(context, _BreatheSettings(preset: _preset, inhaleS: _inh, hold1S: _h1, exhaleS: _exh, hold2S: _h2, haptics: _haptics, targetCycles: _target, tts: _tts));
                  })),
                ],
              ),
              const SizedBox(height: 8),
